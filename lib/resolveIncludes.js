...
var INCBIN_REGEXP = /^[^;\n]*[ \t]\bincbin[ \t]+(?:"([^;"\n]+?)"|'([^;'\n]+?)'|([^ ;'"\n]+)\b)/gmi;
;
function resolveIncludes(entrySource, getFile, baseDir) {
 ...
 */
function createIncludeFromSearchResult(include, isBinary, baseDir, includeDirs, getFile) {
    var uri;
    var contents;
    ...
    var childIncludes = getFile && contents && typeof (contents) === "string" ? resolveIncludes(contents, getFile, path.dirname(uri)) : [];
    return {
        line: include.line,
